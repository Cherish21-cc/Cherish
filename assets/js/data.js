...
window.MEMBERSHIP = {
  ...
  price: {
    ...
    first: "￥9.9",
    regular: "之后 ￥19.9 / 月",
    /* bundle 是可选的打包档。校招是季节性的，一个学生的需求期大概 3—4 个月，
       按月付要手动续 4 次，中间很容易流失（十月拿到 offer，十一月就不续了）。
       打包档对用户更划算、对你一次收满，不需要就把整个 bundle 删掉。*/
    bundle: {
      label: "整个秋招季",
      amount: "￥39.9",
      desc: "9—12 月，一次付清，比按月省 ￥39.8"
    },
...
/* ============================================================
   公司避雷墙（用户投稿 + 人工审核后发布）
   ------------------------------------------------------------
   ⚠️ 这个板块法律风险最高，上线前请先读完这段。

   1) 必须人工审核后再发布，不要做成实时 UGC。
      《民法典》第 1195 条规定了「通知—删除」规则：企业发函要求删除
      而你不及时处理，要承担连带责任。实时发布等于把审核义务架空。

   2) 只发可核实的客观事实，不发主观评价和辱骂。
      可以发：「宣讲会说工作地在武汉，签约后派驻至县级项目部」
      不要发：「XX 公司就是黑心血汗工厂」
      前者是事实陈述，后者是商业诋毁，企业可以直接起诉。

   3) 绝对不要点名个人。HR、面试官的姓名、部门、长相一律不写。

   4) 每条都要标注来源状态（verified 字段），不要让读者误以为
      未核实的投稿是站方结论。

   5) ★ 最重要：避雷内容必须永久免费，绝对不能放进付费区。
      「收费才能看企业负面信息」会被认定为以盈利为目的的商业诋毁，
      性质比单纯转述严重得多，也会毁掉整个站的公信力。

   items 初始为空 —— 我不会替你编造对真实公司的负面指控，
   那是诽谤。等真实投稿进来、你审核过了再往里加。
   ============================================================ */
window.WARNINGS = {
  /* 关着。建好投稿表单、并读完上面那五条规则之后，
     把这里改成 true 并填 formUrl 即可。*/
  enabled: false,
  formUrl: "",          // ← 投稿表单链接（腾讯问卷 / 金数据 / 问卷星都行）
  contactNote: "如果你是企业方，认为某条内容不实，可以通过同一个表单提交更正说明，核实后会修改或删除。",
  items: [
    /* 审核通过后照这个格式往里加，最新的放最前面：
    {
      company: "某公司",
      date: "2026-10-12",
      tag: "签约",                       // 流程 / 签约 / 派驻 / 薪酬 / 宣讲不符
      body: "宣讲会明确说工作地在武汉，三方到手后派驻地写的是县级项目部。",
      verified: false                    // true = 你已交叉核实过多份投稿
    },
    */
  ]
};
...
window.CHANGELOG = [
  {
    ...
    items: [
      ...
      "企业名录顶部会显示最近 7 天内的更新，超过 7 天自动隐藏",
